...
import { 
  ...
  obtenerMetricasDeAsistenciaPorComision // <-- Importamos el nuevo endpoint
...
export default function MetricasLargoComision({ comisionId, numeroComision }: Props) {
  ...
  useEffect(() => {
      ...
      obtenerMetricasDeAsistenciaPorComision(comisionId)
        ...
        .catch(() => toast.error("No se pudieron cargar las métricas de asistencia"))
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-card rounded-lg shadow-card border border-border p-5">
        <h3 className="text-sm font-semibold text-foreground mb-4">Evolución de asistencia por encuentro</h3>
            ...
            <BarChart data={barData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
              ...
              <Tooltip 
                ...
                labelFormatter={(label, payload) => {
                  if (payload && payload.length > 0) return payload[0].payload.tituloOriginal;
                  return label;
                }}
              ...
              <Bar dataKey="Faltas Justificadas" stackId="a" fill={COLOR_JUSTIFICADA} />
              {/* La última barra apilada lleva los bordes superiores redondeados */}
              <Bar dataKey="Ausentes" stackId="a" fill={COLOR_AUSENTE} radius={[4, 4, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        )}
      </div>
          ...
          <div className="flex items-center justify-center">
            ...
            <div className="ml-4 space-y-2">
              <div className="flex items-center gap-2 text-sm">
                <div className="w-3 h-3 rounded-full" style={{ backgroundColor: COLORS_PIE[1] }} />
                <span className="text-foreground font-medium">
                  Activos: {activosCount}
                </span>
              </div>
              <div className="flex items-center gap-2 text-sm">
                <div className="w-3 h-3 rounded-full" style={{ backgroundColor: COLORS_PIE[0] }} />
                <span className="text-foreground font-medium">
                  Bajas: {bajasCount}
                </span>
              </div>
