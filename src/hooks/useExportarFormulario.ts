...
export function useExportarFormulario(
  refFormulario: React.RefObject<HTMLDivElement>,
  nombrePersonalizado?: string,
) {
  const nombreArchivo =
    nombrePersonalizado ??
    `formulario_feedback_anonimo_${new Date().toISOString().split("T")[0]}`;

  const exportarPDF = async () => {
    if (!refFormulario.current) return;

    const canvas = await html2canvas(refFormulario.current, {
      scale: 2,
      useCORS: true,
    ...
    pdf.text(nombreArchivo, margin, margin);
