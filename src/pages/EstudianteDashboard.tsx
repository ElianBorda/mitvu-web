...
export default function EstudianteDashboard({ unenrolled = false }: { unenrolled?: boolean }) {
  ...
  if (loading) return <p className="text-muted-foreground text-sm">Cargando comisión...</p>;
  
  if (hasNoComision || unenrolled) {
    return (
        ...
        <div className="flex items-center justify-center min-h-[60vh]">
          <div className="bg-card border border-border rounded-xl shadow-card px-8 py-12 max-w-lg w-full text-center">
            <h2 className="text-xl sm:text-2xl font-semibold text-foreground">
              {hasNoComision && "No posée comisión"}
              {unenrolled && "Te diste de baja del taller"}
            </h2>
            <p className="text-sm text-muted-foreground mt-3">
              {hasNoComision && "El estudiante no pertenece a ninguna comisión. Contacte con el administrador para realizar la inscripción."}
