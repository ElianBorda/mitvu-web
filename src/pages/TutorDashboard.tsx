...
import CommissionCard from "@/components/ComisionCard";
...
export default function TutorDashboard() {
  ...
  const handleSidebarClick = (itemId: string) => {
    ...
    if (role === "student" && itemId === "calendar") {
      ...
      <div className="flex flex-col lg:flex-row gap-6">
        <div className="flex-1 min-w-0">
          ...
          <div className="space-y-4">
            {comisiones.map(c => (
              <CommissionCard key={c.id} commission={c} onClick={() => setComisionSeleccionada(c)} />
            ))}
          </div>
        </div>
